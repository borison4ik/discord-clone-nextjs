          ...
          {!fileUrl && !isEditing && (
            <p
              className={cn(
                ...
                deleted && 'italic line-through text-zinc-500 dark:text-zinc-400 text-xs mt-1',
              )}>
              {content}
              {isUpdated && !deleted && (
      ...
      {canDeleteMessage && (
        <div className='hidden group-hover:flex items-center gap-x-2 absolute p-1 -top-2 right-5 bg-white dark:bg-zinc-800 border rounded-sm'>
          ...
          <ActionTooltip label='Delete'>
            <Trash className='h-4 w-4 cursor-pointer text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition' />
          </ActionTooltip>
        </div>
      )}
    </div>
