...
export const MembersModal = () => {
  ...
  const isModalOpen = isOpen && type === modalTypes.MEMBERS;
  ...
  const onKick = async (memberId: string) => {
    try {
      ...
      router.refresh();
      onOpen(modalTypes.MEMBERS, { server: response.data });
    } catch (error) {
      console.log(error);
    } finally {
      setLoadingId('');
            ...
            <div key={member.id} className='flex items-center gap-x-2 mb-6'>
              ...
              <div className='flex flex-col gap-y-1'>
                <div className='text-xs font-semibold flex items-center'>
                  {member.profile.name === 'null null' ? member.profile.email.split('@')[0] : member.profile.name} {roleIconMap[member.role]}
                </div>
              ...
              {loadingId === member.id && <Loader2 className='h-4 w-4 ml-auto animate-spin text-zinc-500' />}
            </div>
          ))}
        </ScrollArea>
      </DialogContent>
